...
export const useRegistrationForm = ({
  ...
  const validateField = (name: string, value: string, code?: string) => {
    const error = validateFieldLogic(
      name,
      value,
  ...
  const handleInputChange = (
    ...
    if (name === 'session') {
      ...
      if (selectedSession?.fixedDate) {
        const times = selectedSession.fixedTime
          ? selectedSession.fixedTime.split(',').sort()
          : [];
        let timeToUse =
          times.length > 0 ? times[0] : timeslotConfig.generalStart;
        if (timeToUse.length === 4 && timeToUse.includes(':'))
          timeToUse = '0' + timeToUse;
        newPickupTime = `${selectedSession.fixedDate} ${timeToUse}`;
      } else {
        newPickupTime = findEarliestSlot(
          sessions,
          timeslotConfig,
          generalTimeSlots,
          specialTimeSlots,
  ...
  const handleDateChange = (date: Date | null) => {
    if (date) {
      ...
      setFormData((prev) => ({
        ...prev,
        pickupTime: formatDateTimeMinute(adjustedDate)
      }));
    }
  };
